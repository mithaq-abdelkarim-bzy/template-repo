...
def sch_rate_change(cds):
    ...
    cds.override_node_properties("cds/layers/rate_change/rate_change/comments", {"async_output": ["rarc_task"]})
    ...
    cds.extend_node_rater_defined("cds/layers/rate_change", {
        ...
        **{
            ...
            for item, label, format_ in zip(
                ...
                [copy.deepcopy(thousands_format()) for _ in range(4)] + [percent_format()]
            )
        },
    ...
    # Add async_input to relevant fields - NOTE: overrides previous overrides so not working as intended
    # buckets = rate_change_buckets()

    # for bucket in buckets.keys():
    #     if bucket != "brokerage":
    #         for field in buckets[bucket]:
    #             cds.override_node_properties(field, {"async_input": ["rarc_task"]})