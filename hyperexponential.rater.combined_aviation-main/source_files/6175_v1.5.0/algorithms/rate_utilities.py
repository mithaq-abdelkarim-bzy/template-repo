...
def sanitize_and_sort_expiring_list_by_renewal(
    ...
    sorting_key, 
    fillna=False, 
    remove_bulk_pricing=False,
    remove_duplicates=False
):
    """
    Sort the expiring list to follow the order of the renewal list based on a given sorting key.
    
    - If fillna is True, empty or placeholder values (None, 'tbc', or 'tba') in the expiring list for the
      sorting key are replaced with a unique sentinel value.
    - If remove_bulk_pricing is True, items with a 'no_of_aircraft' value greater than 1 will have their
      sorting key replaced with a unique sentinel value.
    ...
    # Process fillna and bulk pricing removal conditions
    for idx, d in enumerate(expiring_list):
        # Apply fillna logic if enabled
        if fillna:
            if d.get(sorting_key) is None or d.get(sorting_key).strip().lower() in {"tbc", "tba"}:
                d[sorting_key] = f"{SENTINEL}_{idx}"
                flag_fillna = True
                continue  # Skip further checks if fillna condition is met

        # Apply bulk pricing removal if enabled
        if remove_bulk_pricing:
            if d.get("no_of_aircraft") > 1:
                d[sorting_key] = f"{SENTINEL}_{idx}"
                flag_bulk = True
    ...
    warnings = []
    if fillna and flag_fillna:
        warnings.append("Null/TBA/TBC registrations are present in the expiry fleet.")
    if remove_bulk_pricing and flag_bulk:
        warnings.append("Bulk-rated registrations are present in the expiry fleet.")
    if remove_duplicates and flag_exp_duplicates:
        warnings.append("Duplicate registrations are present in the expiry fleet.")
    if remove_duplicates and flag_renewal_duplicates:
        warnings.append("Duplicate registrations are present in the renewal fleet, and the same registration is present in the expiry fleet.")        
