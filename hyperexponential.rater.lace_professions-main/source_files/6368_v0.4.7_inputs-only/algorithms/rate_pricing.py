...
from algorithms.rate_constants import (retention_split_notes,get_territory_groups,get_fx_rate,get_fx_rate_base,get_base_params,get_tech_params,get_nmp_load,get_inflation_tables,
                                        ...
                                        client_details_lawyers_to_dataframe,client_details_aec_to_dataframe, hover_info
                                        )
...
def claims_recoveries_dropdown_calc(hxd, layers, bool_is_addl: bool) -> pd.DataFrame:
    ...
    # Tower
    result = {}
    rec_across_tower_sum = defaultdict(int) # recoveries up to and including the prior claim for a given layer
    for index, layer in enumerate(layers):
        if layer.include:
            # Get layer limits
            layer_name = layer.name
            layer_eec_limit = (layer.limit_eec or 0) / fx_rate
            layer_agg_limit = np.inf if not layer.limit_agg else layer.limit_agg

            # working arrays
            layer_rec_arr = defaultdict(int)   #automatically zero
            running_total_recovery = defaultdict(int)

            for idx, claim in enumerate(claims_path):
                # Post retention incurred
                pri = claim.RDC_False.final_total_incurred    # always picks Retention not applied on Defence Cost

                if (claim.policy_year_estimated is not None) & (pri is not None):
                    # calculate recovery
                    recovery = min(pri - rec_across_tower_sum.get(idx,0), layer_eec_limit, layer_agg_limit - running_total_recovery.get(claim.policy_year_estimated, 0))

                    # Update the working arrays
                    running_total_recovery[claim.policy_year_estimated] += recovery
...
def develop_claims(hxd):
    ...
    df_layers_rec_addl_dropdown = claims_recoveries_dropdown_calc(hxd, hxd.cds.layers_addl, True)   #Not seupt properly for the additional Layers
        ...
        for layers in [hxd.cds.layers, hxd.cds.layers_addl]:
            for layer in layers:
                if layer.include:
                    ...
                    layer.experience_weighting = df_summary[df_summary["layer"]==name]["credibility_weighting"].iloc[0]
    ...
    def calc_layer_structure(layers):
        for idx, layer in enumerate(layers):
            if layer.include:
                ...
                experience_weighting = layer.experience_weighting or 0
                ...
                layer.blended_model_net_rate = (experience_rate * experience_weighting + layer.exposure_rate_incl_adj * (1 - experience_weighting)) * (1 + cat_load + nmp_load)
                ...
                layer.blended_uw_net_rate = ((1 - layer.uw_experience_weighting) * layer.exposure_rate_incl_adj + layer.uw_experience_weighting * experience_rate) * (1 + cat_load + nmp_load)
                ...
                layer.technical_premium_100 = ((layer.expected_loss_cost_net * (1 + tp.che) + tp.fixed_expense) / (1 - tp.variable_expense + tp.investment_income - tp.ri_cost - tp.roc * tp.capital_cost)) / (1 - brokerage)
                layer.benchmark_premium_100 = ((layer.expected_loss_cost_net or 0) / 0.7) / (1 - brokerage)
                ...
                layer.technical_premium_ryv = ((layer.expected_loss_cost_net * (1 + elevated_risk_year_load) * (1 + tp.che) + tp.fixed_expense) / (1 - tp.variable_expense + tp.investment_income - tp.ri_cost - tp.roc * tp.capital_cost)) / (1-brokerage)
...
def populate_ave_chart(hxd):
    ...
    sim_dict = {
        ...
        "loss": [100000, 2000000, 4000000]
    }

    act_dict = {
        "percentile": [0.1, 0.5, 0.9],
        "loss": [150000, 2500000, 5000000]
    }
