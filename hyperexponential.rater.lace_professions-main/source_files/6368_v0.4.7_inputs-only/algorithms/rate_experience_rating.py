...
from algorithms.rate_constants import max_layers, claims_notes
...
def rate_experience_rating(hxd):
    ...
    for index in range(1, num_layers + 1):
            ...
            if er.use_experience_rating_agg:
                ...
                layer_quoted_premium_100 = hxd.cds.layers[index - 1].quoted_premium_100
                setattr(
                    getattr(er, f"summary_layer_{index}")
                    , "expected_losses"
                    , loss_ratio * layer_quoted_premium_100 if (not layer_quoted_premium_100 is None) else None
                    )
