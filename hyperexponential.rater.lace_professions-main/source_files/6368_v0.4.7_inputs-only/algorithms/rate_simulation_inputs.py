...
def get_model_params(hxd, profession: str, expected_exposure: float):
    """
    ...
    df_ter = df_ter[df_ter["weighted"] > 0]       # filter out 
    ...
    df_aop = df_aop[df_aop["weighted"] > 0]    # filter out
    ...
    aop_sev_mult_total = df_aop["sev_mult"].dot(df_aop["weighted"]) 
    ...
    non_geog_freq_mult = aop_freq_mult_total * uw_factor * rdi_freq_mult
    non_geog_sev_mult = aop_sev_mult_total
    ...
    df_ModelParamsFinal = df_ter[["territory","ded_class","lambda_new","dcaf","dcop","dcp1","mu_adj","sigma_adj"]]
    df_ModelParamsFinal.rename(columns={
        "territory": "Area",
        "ded_class": "Class",
        "lambda_new": "En",
        "dcaf": "DCAF",
    ...
    df_ModelParamsUnadjustedFinal = df_ter[["territory", "ded_class","lambda_new_unadj","dcaf","dcop","dcp1", "mu_adj", "sigma_unadj"]]
    df_ModelParamsUnadjustedFinal.rename(columns={
        "territory": "Area",
        "ded_class": "Class",
        "lambda_new_unadj": "En",
        "dcaf": "DCAF",
...
def get_quotes_params(hxd):
    ...
    cols_to_convert = ["Leec", "Lagg", "Xeec", "Xagg", "RTC_Lim", "RTC_Agg"]
    df[cols_to_convert] = df[cols_to_convert].fillna(0) / risk_fx_rate
    return df
...
def build_simulation_input_snapshot(
    ...
    *,
    ...
    if df_model_params is None:
        df_model_params, _ = get_model_params(hxd, profession, expected_exposure)
    snapshot = {
