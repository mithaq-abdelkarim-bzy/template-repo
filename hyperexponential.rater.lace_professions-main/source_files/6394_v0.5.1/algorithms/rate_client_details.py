...
from algorithms.rate_constants import (exposure_details_notes, get_inflation_tables, client_details_notes, 
    project_type_map, project_type_map_reporting, project_type_reporting_inputoutputmap, incomplete_column, complete_column, profession_dict, project_type_parent_categories, get_aop_aec_parent_categories, get_aop_lpl_parent_categories)
import datetime
...
def get_ipt_nodes():
    ...
    return nodes_1, nodes_2, sub_nodes, sub_total_fields
...
def AEC_ipt_summary_totals(hxd):
    nodes_1, nodes_2, sub_nodes, sub_total_fields = get_ipt_nodes()
...
def write_ipt_reporting(hxd):
    ...
    for year in years:
        for node in nodes:
            reporting_node = project_type_reporting_inputoutputmap.get(node, node)   # defaults to node if it's not in the dict
            setattr(getattr(path.individual_project_types_reporting, reporting_node), year, getattr(getattr(path.individual_project_types, node), year))
...
def AEC_aop_totals(hxd):
    path = hxd.cds.exposure.granular.client_details_AEC
    skip_aop_validation = hxd.cds.profession == "Contractors"
    ...
    if is_negative_check and not skip_aop_validation:
