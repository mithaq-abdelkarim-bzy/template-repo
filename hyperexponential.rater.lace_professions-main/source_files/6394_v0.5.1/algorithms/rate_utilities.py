...
TRIANGLE_KEYS = {"use_triangle", "use_output_triangle", "triangle", "output_triangle"}
def map_expiring_data(expiring_data):
    def strip(obj):
...
def write_pd_to_hxd(df, list_node, output_cols_to_write, override_cols_to_write = []):
    ...
    n = min(df.shape[0], len(list_node))
...
def yearfrac(start_date, end_date):
    """Replicates Excel YEARFRAC with Actual/Actual basis."""
