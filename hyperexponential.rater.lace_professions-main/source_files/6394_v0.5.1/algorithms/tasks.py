...
from algorithms.rate_change.rarc_layer_with_simulation import task_rarc_layer_with_simulation
...
def rarc_task(hxd, progress):
    task_rarc_layer_with_simulation(hxd, progress)
...
def run_simulation_charts_task(hxd, progress):
    ...
    df_results = df_results.rename(columns={"expected_exposure":"revenue"})
    ...
    df_results_addl = df_results_addl.rename(columns={"expected_exposure":"revenue"})
...
def run_simulation_task(hxd, progress):
    ...
    df_quote_params_addl = get_quotes_params_addl(hxd)
    df_ded_params = get_ded_params(hxd)
    df_ModelParamsFinal, df_ModelParamsUnadjustedFinal = get_model_params(hxd, profession, expected_exposure, False)
    ...
    #print 
    print(df_quote_params.to_string())
    print(df_ded_params.to_string())
    print(df_ModelParamsFinal.to_string())
    print(df_ModelParamsUnadjustedFinal.to_string())

    # Run the simulation
    df_modeloutputs, df_addmodeloutputs, sim_loss_dict = run_simulation(
        ded_params = df_ded_params, 
        model_params = df_ModelParamsFinal, 
        quotes_params = df_quote_params, 
        add_quotes_params = df_quote_params_addl, 
    ...
    for idx, layer in enumerate(hxd.cds.layers):
        if layer.include:
            ...
            layer.exposure_rate = df_modeloutputs["BookRate"].iloc[counter] * fx_rate
            counter += 1
    ...
    for idx, layer in enumerate(hxd.cds.layers_addl):
        if layer.include:
            ...
            layer.exposure_rate = df_addmodeloutputs["BookRate"].iloc[counter] * fx_rate
            counter += 1

    # AvE Chart - Sim Results
    df = sim_loss_dict["fgu_claims"]
    hxd.cds.ave_chart.data.points_exposure = [{}] * len(df)    # required hack to initialise the list to the correct length
    utils.write_pd_to_hxd(
        df, 
        hxd.cds.ave_chart.data.points_exposure,
        ["percentile", "loss"]
    )