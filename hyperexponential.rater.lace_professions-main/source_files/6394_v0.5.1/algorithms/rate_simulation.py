...
def run_simulation(
    ...
    collect_losses : bool, optional
        When True, a third element is appended to the return value: a dict of
        pooled individual-claim loss amounts (no territory distinction),
        suitable for building loss/percentile distributions via
        ``loss_distribution``. Keys:

            "fgu_claims"          : 1-D ndarray of every ground-up (indemnity +
                                    defence) simulated claim amount, before any
                                    retention or layer is applied.
            "after_layers_claims" : 1-D ndarray of each claim's loss ceded to
                                    the tower (summed across all tower quote
                                    layers, after retention/excess/limit/agg).
    ...
    tuple of (model_outputs, add_model_outputs) : each a pd.DataFrame
        When ``collect_losses=True`` the tuple has a third element,
        ``sim_losses`` (dict, described above).
    ...
    def _col(df, name, fill=0.0):
        ...
        Only NaN is replaced — +/-Inf must be preserved as-is (Xagg/Lagg
        legitimately use Inf to mean "no aggregate attachment/limit", and
        apply_quotes() branches on `xa == np.inf` explicitly). The default
        np.nan_to_num() would otherwise silently replace +/-Inf with the
        float64 max/min, which is well within float64 range and does not
        raise, but its magnitude relative to real claim sizes makes any
        subtraction against it a silent no-op — disabling per-claim excess
        deduction whenever an excess-of-loss layer combines Xeec > 0 with
        Xagg = Inf.
        """
        return np.nan_to_num(
            df[name].to_numpy(dtype=float), nan=fill, posinf=np.inf, neginf=-np.inf
        )
    ...
    # Pooled ground-up (FGU) individual claim amounts, no territory distinction.
    fgu_claim_values: list[np.ndarray] = []
    ...
    for territory in territories:
        ...
        # Ground-up (FGU) claims: the raw lognormal draws before any split,
        # retention or layer. Pool the non-zero entries (padding rows are 0).
        if collect_losses:
            fgu_claim_values.append(individual_claims[individual_claims > 0].ravel())
    ...
    # Matches R's DCIA-loop construction of `defense_matrix`: the *first*
    # retention class contributes the retention-adjusted defence split
    # (`loss_D` from retention_apply3), while any additional retention
    # classes stacked on top fall back to the raw pre-retention defence costs
    # (i > 1 only). defence_parts[0] is already computed above — this was
    # previously discarded in favour of the raw lvD matrix for every class.
    defence_stack_parts = [defence_parts[0]]
    defence_stack_parts += [lvD[rc][0] for rc in retention_classes[1:]]
    defence_matrix = np.vstack(defence_stack_parts) 
    ...
    model_df = pd.DataFrame(model_outputs, columns=_cols)
    add_model_df = pd.DataFrame(add_model_outputs, columns=_cols)

    if collect_losses:
        fgu_claims = (
            np.concatenate(fgu_claim_values)
            if fgu_claim_values
            else np.empty(0, dtype=float)
        )

        fgu_claims_df = pd.DataFrame({"loss": fgu_claims})
        ...
        if n == 0:
            fgu_claims_df["percentile"] = []
        elif n == 1:
            fgu_claims_df["percentile"] = [1.0]
        else:
            fgu_claims_df["percentile"] = np.linspace(1.0, 0.0, n)

        max_points = 1000
        if len(fgu_claims_df) > max_points:
            idx = np.linspace(0, len(fgu_claims_df) - 1, max_points, dtype=int)
            fgu_claims_df_plot = fgu_claims_df.iloc[idx].reset_index(drop=True)
        else:
            fgu_claims_df_plot = fgu_claims_df.copy()

        sim_loss_dict = {
            "fgu_claims": fgu_claims_df_plot,
            "post_retention_claims": [],    # placeholder in case required
            "post_layers_claims": []        # placeholder in case required
        }

        #print(fgu_claims_df_plot)
        #print(len(fgu_claims_df_plot))
        return model_df, add_model_df, sim_loss_dict


    return model_df, add_model_df
