...
def _scalar_value(value, default=0):
    """Return a scalar value when a calculation has produced a pandas object."""
    if value is None:
        return default
...
def calculate_premiums(calculated_level, hxd, nmp_load, che, fixed_exp, technical_lr, benchmark_lr, var_exp, inv_inc, cost_of_ri, ri_rec, capital_req, rf): # Edit v0.3.0
    ...
    if hxd.cds.standard_fields.is_rater_priced and calculated_level.quoted_premium_100:
        ...
        #expected_loss_cost_net = _scalar_value(calculated_level.expected_loss_cost_net, 0)
        #expected_loss_cost_net_pre = _scalar_value(calculated_level.expected_loss_cost_net_pre, 0)

        premium = 1
        if calculated_level.status == "Bound":
            premium = _scalar_value(calculated_level.bound_premium_100, 1)
        else:
            premium = _scalar_value(calculated_level.quoted_premium_100, 1)
        ...
        #calculated_level.pflr = ratio(expected_loss_cost_net, premium)
        #calculated_level.pflr_pre_uw_adj = ratio(expected_loss_cost_net_pre, premium)
        #calculated_level.uw_adj_impact = ratio(expected_loss_cost_net, expected_loss_cost_net_pre) - 1
