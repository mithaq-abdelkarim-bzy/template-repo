...
from algorithms.rate_constants import get_aop_lawyers_count, get_aop_ae_count, project_type_map, project_type_map_reporting, project_type_parent_categories, get_aop_aec_parent_categories, get_aop_lpl_parent_categories
...
def sch_exposure_details(cds):
    ...
    def output_float_node(value, bool_is_pcnt, year_label):
        if bool_is_pcnt:
            node = hx.Float(mode="output",  async_input=["run_simulation_task","rarc_task"], view={"label": year_label,"format": {"output": "percent", "mantissa": 1}})
        else:
            node = hx.Float(mode="output",  async_input=["run_simulation_task","rarc_task"], view={"label": year_label,"format": {"thousandSeparated": True, "mantissa": 0}})
    ...
    cds.extend_node_rater_defined("cds/exposure/granular", {        
        ...
        "client_details_AEC": hx.Structure(children={
            ...
            "individual_project_types_reporting": hx.Structure(children={
                **{f"{key}":hx.Structure(children={
                    "year_0_pcnt": output_float_node(value, True, "year_0"),
                    "year_0_value": output_float_node(value, False if key != "total" else True, "year_0"),
                    "year_1_pcnt": output_float_node(value, True, "year_1"),
                    "year_1_value": output_float_node(value, False if key != "total" else True, "year_1"),
                    "year_2_pcnt": output_float_node(value, True, "year_2"),
                    "year_2_value": output_float_node(value, False if key != "total" else True, "year_2"),
                    "year_3_pcnt": output_float_node(value, True, "year_3"),
                    "year_3_value": output_float_node(value, False if key != "total" else True, "year_3"),
                    "year_4_pcnt": output_float_node(value, True, "year_4"),
                    "year_4_value": output_float_node(value, False if key != "total" else True, "year_4"),
                    "year_5_pcnt": output_float_node(value, True, "year_5"),
                    "year_5_value": output_float_node(value, False if key != "total" else True, "year_5"),
                    "weighted": hx.Float(mode="output", async_input=["run_simulation_task", "rarc_task"], view={"label": "Weighted % Split","format": {"output": "percent", "mantissa": 1}}),
                    "frequency": hx.Float(mode="output",view={"label": "Frequency" if key != "total" else "Lambda\n(Mean Freq)", 
                                                                "format":utils.percent_format(1) if key != "total" else {"thousandSeparated": True, "mantissa": 3}, 
                                                                "group": "Loading" if key != "total" else "Adjusted"}),
                ...
                for key, value in project_type_map_reporting.items()}
            })
        }),
