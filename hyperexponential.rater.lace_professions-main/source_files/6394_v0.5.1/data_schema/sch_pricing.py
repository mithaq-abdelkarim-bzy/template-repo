...
from algorithms.rate_constants import get_territory_groups, territory_labels, get_all_layer_names, new_column
...
def sch_pricing(cds):
    ...
    # Extend the below as required.
        ...
        "retention_split": hx.List(mode="input", default_element_count=5, children={
            ...
            #"defence_cost_bool": hx.Bool(mode="input", async_input=["run_simulation_task"], optionality="required", default = False, view={"label":"Applies to Defence Cost?"})
        }),
        ...
        "ave_chart": hx.Structure(
            children={
                        "data": hx.Structure(
                            children = {
                                "label_exposure": hx.Str(mode="output", view={"label": "Simulated Loss"}),
                                "label_experience": hx.Str(mode="output", view={"label": "Actual Loss"}),
                                "points_exposure": hx.List(
                                    mode="output",
                                    async_output=["run_simulation_task"],
                                    children={
                                        "percentile": hx.Float(mode="output", async_output=["run_simulation_task"], view={"label": "Percentile", "format": {"output": "percent", "mantissa": 1}}),
                                        "loss": hx.Float(mode="output", async_output=["run_simulation_task"], view={"format": thousands_format(1), "chart": {"series_type": "pointline"}}),
                                    },
                                ),
                                "points_experience": hx.List(
                                    mode="output",
                                    children={
        ...
        "pbu_chart": hx.Structure(
            children={
                        "data": hx.Structure(
                            children = {
                                "technical_premium": hx.Structure(
                                    children={
                                ...
                                "benchmark_premium": hx.Structure(
                                    children={
                                        "brokerage": hx.Float(mode="output"),
                                        "profit_load": hx.Float(mode="output"),
                                        "expenses": hx.Float(mode="output"),
    ...
    cds.extend_node_rater_defined("cds/layers", {
        ...
        "excess_eec": hx.Float(mode="output", async_input=["rarc_task", "run_simulation_task"], view={"label": "AOC", "format": {"thousandSeparated": True, "mantissa": 0}, "group":"Excess"}),
        "excess_agg": hx.Float(mode="output", async_input=["rarc_task", "run_simulation_task"], view={"label": "Aggregate", "format": {"thousandSeparated": True, "mantissa": 0},"group":"Excess"}),
        ...
        # Model Net Rate per mil Revenue
        ...
        "blended_model_net_rate": hx.Float(mode="output", view={"label": "Blended Model\nNet Rate",  "format": {"thousandSeparated": True, "mantissa": 0}, "group": new_column+"Post Adjustments"}),           # post
        "expected_loss_cost_net": hx.Float(mode="output", view={"label": "Expected\nNet Loss Cost",  "format": {"thousandSeparated": True, "mantissa": 0}, "group": new_column+"Post Adjustments"}),           # post
        ...
        "bpi_quoted_100": hx.Float(mode="output", optionality="optional",  view={"label": "Quoted BPI", "format": {"output": "percent", "mantissa": 1},"group":new_column+"100% Gross Share Excluding UW Adj"}),
        "bpi_bound_100": hx.Float(mode="output", optionality="optional",  view={"label": "Bound BPI", "format": {"output": "percent", "mantissa": 1},"group":new_column+"100% Gross Share Excluding UW Adj"}),
        "tpi_quoted_100": hx.Float(mode="output", optionality="optional",  view={"label": "Quoted TPI", "format": {"output": "percent", "mantissa": 1},"group":new_column+"100% Gross Share Excluding UW Adj"}),
        "tpi_bound_100": hx.Float(mode="output", optionality="optional",  view={"label": "Bound TPI", "format": {"output": "percent", "mantissa": 1},"group":new_column+"100% Gross Share Excluding UW Adj"}),
        "bpi_quoted_100_incl_adj": hx.Float(mode="output", optionality="optional",  view={"label": "Quoted BPI", "format": {"output": "percent", "mantissa": 1},"group":new_column+"100% Gross Share Including UW Adj"}),
        "bpi_bound_100_incl_adj": hx.Float(mode="output", optionality="optional",  view={"label": "Bound BPI", "format": {"output": "percent", "mantissa": 1},"group":new_column+"100% Gross Share Including UW Adj"}),
        "tpi_quoted_100_incl_adj": hx.Float(mode="output", optionality="optional",  view={"label": "Quoted TPI", "format": {"output": "percent", "mantissa": 1},"group":new_column+"100% Gross Share Including UW Adj"}),
        "tpi_bound_100_incl_adj": hx.Float(mode="output", optionality="optional",  view={"label": "Bound TPI", "format": {"output": "percent", "mantissa": 1},"group":new_column+"100% Gross Share Including UW Adj"}),
        # Status
        # Policy Reference
        # written_line
    ...
    # cds.override_node_properties("cds/layers/technical_premium_100", {"async_output":["run_simulation_task"]})
    # cds.override_node_properties("cds/layers/benchmark_premium_100", {"async_output":["run_simulation_task"]})
    # cds.override_node_properties("cds/layers/bound_premium_100", {"async_input":["run_simulation_task"]})
    # cds.override_node_properties("cds/layers/quoted_premium_100", {"async_input":["run_simulation_task"]})
    # cds.override_node_properties("cds/layers/written_line", {"async_output":["run_simulation_task"]})
    ...
    cds.extend_node_rater_defined("cds", {
        "layers_addl": hx.List(mode="input", async_input=["rarc_task"], default_element_count=3, max_element_count=5,children={
            ...
            "name": hx.Str(mode="output",  async_input=["rarc_task", "run_simulation_task"], view={"label": "UW Adjustment", "format": {"thousandSeparated": True, "mantissa": 0}}),
            "include": hx.Bool(mode="input", async_input=["rarc_task", "run_simulation_task"], optionality="required", default=True, view={"label": "Include?"}), 
            "brokerage": hx.Float(mode="input", async_input=["rarc_task", "run_simulation_task"], 
                                    validation={"min_value":0.0, "max_value":0.9999},
                                    optionality="optional", default=0, 
                                    view={"label": "Brokerage", "format": {"output": "percent", "mantissa": 1}, "options": {"read_only": {"read_only": True}}}),
            "sim_output_uw_adj":hx.Structure(children={
                "exposure_premium": hx.Float(mode="output", async_output=["run_simulation_task"], view={"label": "Exposure Premium\nper $m revenue/fees", "format": {"thousandSeparated": True, "mantissa": 0}}),
                "average_freq": hx.Float(mode="output", async_output=["run_simulation_task"], view={"label": "Av.Frequency\nper $m revenues/fees",  "format": {"output": "percent", "mantissa": 1}}),
                "average_defense_cost_freq": hx.Float(mode="output", async_output=["run_simulation_task"], view={"label": "Av Defense Cost Freq\nper $m revenue/fees",  "format": {"output": "percent", "mantissa": 1}}),
