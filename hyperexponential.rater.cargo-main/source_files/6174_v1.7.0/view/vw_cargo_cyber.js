...
function TransitCollection(props) {
  return <HX.Collection with="coverages/cargo_cyber_transit" horizontal {...props} />;
}

function StorageCollection(props) {
  return <HX.Collection with="coverages/cargo_cyber_storage" horizontal {...props} />;
}

function vw_cargo_cyber(scale) {
  return (
    <HX.Page title="Cargo Cyber" viewScale={scale} shownBy="cds/cover_selection/is_cargo_cyber_combined">
      <HX.Section title="">
        <HX.Button title="Load Cargo Input" task="load_cargo_input" shownBy="cds/cover_selection/is_cargo_cyber"></HX.Button>
      </HX.Section>

      <HX.Section title="Cargo Cyber - Transit" defaultCollapsed>
        <HX.With context={{ type: "list", path: "cds/layers", index: 0 }}>
          <HX.Pane>
            ...
            <HX.Section title="Cargo Cyber - Conveyance" collapsible={false}>
              <HX.Pane flow="right">
                <HX.Collection with="coverages/cargo_cyber_transit" fields={["conv_air", "conv_land", "conv_sea"]} />
                <HX.Collection with="coverages/cargo_cyber_transit" fields={["conv_air_factor", "conv_land_factor", "conv_sea_factor", "conv_factor"]} />
              </HX.Pane>
      ...
      <HX.Section title="Cargo Cyber - Storage" defaultCollapsed>
        <HX.With context={{ type: "list", path: "cds/layers", index: 0 }}>
          <HX.Pane>
            ...
            <HX.Section title="CAT Exposure by Country" defaultCollapsed>
              <HX.Table
                data={["coverages/cargo_cyber_storage/countries"]}
