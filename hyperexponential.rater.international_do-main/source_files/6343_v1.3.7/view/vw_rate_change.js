...
function vw_loop_rate_change_mmp() {
  const objects = [];

  objects.push(
    <HX.Section title={"Renewal MMP Total"} shownBy={"cds/risk_information/mmp_flag"}>
      ...
      <HX.Section title="Fetch Expiring Policy">
        ...
        <HX.Pane>
          <HX.Table
            ...
            data={[{ datum: "cds/large_cap/layers", width: 200 }]}
          ...
          />
          <HX.Table
            title="Deal Status of Renewal"
            data={[{ datum: "cds/mmp/total", width: 200 }]}
            fields={[{ field: "status.read_only" }]}
            transpose
            kb-interactive
