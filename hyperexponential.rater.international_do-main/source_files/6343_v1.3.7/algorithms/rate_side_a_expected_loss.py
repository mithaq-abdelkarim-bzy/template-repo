...
def adr_standalone_expected_loss_side_a(hxd, layer_df, prelim_inputs):

    # ADR Expected Loss Ratio
    adr_elr = 0.25

    # Process Frequency Variables
    adr_variables = freq_sev_func.process_freq(hxd, "ADR Standalone", prelim_inputs, is_side_a = True)
    ...
    adr_model_avg_loss, adr_unity_avg_loss = df_func.calc_layer_avg_loss_side_a(adr_variables, layer_df)

    # Calcualte Model, Primary and Unity Net Expected Losses
    adr_model_net_exp_loss, _ = df_func.calc_layer_net_exp_loss_side_a(adr_model_avg_loss, adr_variables, layer_df)
    _, adr_unity_net_exp_loss = df_func.calc_layer_net_exp_loss_side_a(adr_unity_avg_loss, adr_variables, layer_df)
    ...
    for i in ("model", "unity"):
        ...
        # Intermediary Result
        adr_result = np.maximum(15000, eval(f"adr_{i}_exp_loss"))

        # ADR Standalone Total Expected Loss
        df.loc["adr_expected_loss"] = np.where(prelim_inputs["adr_share"] * prelim_inputs["market_cap_adjusted_usd"] < 10000, 0, adr_result * (1 - np.float64(layer_df.loc["brokerage"])) * adr_elr)

        # ADR Expected Loss Ratio
        df.loc["adr_expected_lr"] = [adr_elr] * df.shape[1]
...
def contagion_expected_loss_side_a(hxd, layer_df, adr_model_expected_loss, adr_unity_expected_loss, prelim_inputs):

    # Contagion Expected Loss Ratio
    cont_elr = 0.25

    # Process Frequency Variables
    cont_variables = freq_sev_func.process_freq(hxd, "Contagion", prelim_inputs, is_side_a = True)

    # Process Severity Variables
    cont_variables = freq_sev_func.process_sev(cont_variables)
    
    # Calculate Model and Primary Expected Average Losses (Unity and Model have the same average loss dataframes)
    cont_model_avg_loss, cont_unity_avg_loss = df_func.calc_layer_avg_loss_side_a(cont_variables, layer_df)

    # Calcualte Model, Primary and Unity Net Expected Losses
    cont_model_net_exp_loss, _ = df_func.calc_layer_net_exp_loss_side_a(cont_model_avg_loss, cont_variables, layer_df)
    _, cont_unity_net_exp_loss = df_func.calc_layer_net_exp_loss_side_a(cont_unity_avg_loss, cont_variables, layer_df)
    ...
    for i in ("model", "unity"):
        ...
        # Intermediary Result
        cont_result = np.maximum(15000, eval(f"cont_{i}_exp_loss"))

        # Contagion Total Expected Loss
        cont_freq = utils.look_up("ContagionFrequency", "ParameterName", "Value", hx.params.tbl_intl_mod)
        df.loc["cont_expected_loss"] = np.where(prelim_inputs["adr_share"] * prelim_inputs["market_cap_adjusted_usd"] < 10000, 0, cont_result * (1 - np.float64(layer_df.loc["brokerage"])) * cont_elr - eval(f"adr_{i}_expected_loss.loc['adr_expected_loss']")) * cont_freq

        # Contagion Expected Loss Ratio
        df.loc["cont_expected_lr"] = [cont_elr] * df.shape[1]
...
def intl_standalone_expected_loss_side_a(hxd, layer_df, prelim_inputs):
    ...
    for i in ("model", "unity"):
        ...
        # Coverage Factor
        cover_factor = utils.look_up("A", "CoverageType", "Factor", hx.params.tbl_intl_cover_type)
        ...
        if i == "model":
            if hxd.cds.modifiers.sca_adj_factors.value is not None:
                sca_adjustment = max(uw_min_adj, 1 + hxd.cds.modifiers.sca_adj_factors_side_a.value)
            else:
        ...
        df.loc["intl_expected_lr"] = [0.5] * df.shape[1]

    return intl_model_expected_loss, intl_unity_expected_loss
...
def intl_large_company_expected_loss_side_a(hxd, layer_df, prelim_inputs):

    # Large Company Expected Loss Ratio
    lc_elr = 0.25

    # Process Frequency Variables
    lc_variables = freq_sev_func.process_freq(hxd, "Large Company", prelim_inputs, is_side_a = True)
    ...
    lc_model_avg_loss, lc_unity_avg_loss = df_func.calc_layer_avg_loss_side_a(lc_variables, layer_df)

    # Calcualte Model, and Unity Net Expected Losses
    lc_model_net_exp_loss, _ = df_func.calc_layer_net_exp_loss_side_a(lc_model_avg_loss, lc_variables, layer_df)
    _, lc_unity_net_exp_loss = df_func.calc_layer_net_exp_loss_side_a(lc_unity_avg_loss, lc_variables, layer_df)
    ...
    for i in ("model", "unity"):
        ...
        df.loc["lc_expected_loss"] = np.where(lc_variables["mcap"] < 1.5e6, 0, eval(f"lc_{i}_net_exp_loss.loc['Net Expected Loss']") * np.float64(private_load) * (1 - np.float64(level_1_adj)) * np.float64(prelim_inputs["high_risk_segment_loading"]))

        # International Large Company Loss Ratio
        df.loc["lc_expected_lr"] = [0.5] * df.shape[1]
