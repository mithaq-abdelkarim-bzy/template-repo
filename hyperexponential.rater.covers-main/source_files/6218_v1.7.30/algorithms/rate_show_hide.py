...
def rate_show_hide(hxd):
    ...
    ########################################################
    cds.show_hide.page.show_sov               = False
    ...
    if hxd.model_state.show_after_landing_page:
        ...
        if cds.risk_info.sov_available              == "Yes":        cds.show_hide.page.show_sov = True 
        ...
        if cds.profit_commission.scenarios[0].additionalfeaturesindicator == "Threshold":
            cds.show_hide.node.pc_standard              = False # we still want to show the basic values
        ...
        cds.show_hide.node.signed_line              = False

        if cds.layers[0].status == "Bound":
            cds.show_hide.node.signed_line = True
        else:
            cds.show_hide.node.signed_line = False
        ...
        if (cds.rating_summary.summary_ratios.attritional.gg_pst_uw_adj.ulr_uw_override is None) == False:
            cds.show_hide.node.rs_att_rationale         = True

        if (cds.rating_summary.summary_ratios.large.gg_pst_uw_adj.ulr_uw_override is None) == False:
            cds.show_hide.node.rs_lrg_rationale         = True

        if (cds.rating_summary.summary_ratios.catastrophe.gg_pst_uw_adj.ulr_uw_override is None) == False:
        ...
        if cds.rating_summary.summary_ratios.attritional.uw_override == True:
            cds.show_hide.node.rs_att_rationale         = True
        ...
        ## 2f) risk information - signed line


        # Exposure section 
        cds.show_hide.node.show_experience  = (cds.show_hide.page.show_sov == False)
