...
def rate_validation_criteria(hxd):
    ...
    if (pc.additionalfeaturesindicator == "Threshold"):
        if ((pc.threshold_bonus_share is None)==False)              and   (pc.threshold_bonus_share < 0 or pc.threshold_bonus_share > 1):
            hx.errors.validation(f"Profit Commission Threshold Share (Risk Information Tab) between 0 and 1 must be entered")

        if ((pc.threshold_lr_cutoff is None)==False)                and   (pc.threshold_lr_cutoff < 0 or pc.threshold_lr_cutoff > 1):
    ...
    if ((cds.profit_commission.consistent_pc_latest_param is None)==True)     or   (cds.profit_commission.consistent_pc_latest_param != "PC Parameters upto-date"):
        hx.errors.validation(f"Profit Commission Calculation is not upto-date please run the calculation")
    ...
    for index, i in enumerate(cds.profit_commission.scenarios):
        if index > 0:   # we already handle the base case (0) under the risk information tab
            ...
            if ( (i.threshold_bonus_share            is None) == False)  and  (i.threshold_bonus_share < 0              or i.threshold_bonus_share > 1):            test_threshold_bonus_share = True
            if ( (i.threshold_lr_cutoff              is None) == False)  and  (i.threshold_lr_cutoff < 0                or i.threshold_lr_cutoff > 1):              test_threshold_lr_cutoff = True
