...
def rate_profit_commission(hxd):
    ...
    signed_line = (cds.layers[0].written_line or 1)
    ...
    for i in range(10):    
        ...
        if pc_i.additionalfeaturesindicator == "Sliding Scale":
            ...
            pc_i.complete = ((  (pc_i.share                                    is None)
                                ...
                                | (pc_i.threshold_bonus_share                is None)
                                | (pc_i.threshold_lr_cutoff                  is None)) == False
                              and (pc_i.share                 >=0 and pc_i.share                 <= 1)
                              and (pc_i.expenses              >=0 and pc_i.expenses              <= 1)
                              and (pc_i.threshold_bonus_share >=0 and pc_i.threshold_bonus_share <= 1)
                              and (pc_i.threshold_lr_cutoff   >=0 and pc_i.threshold_lr_cutoff   <= 1)
    ...
    if (pc.scenarios[0].additionalfeaturesindicator == "Threshold"):
        if (    (pc.scenarios[0].share is None)      or      (pc.scenarios[0].expenses is None)      
            or  (pc.scenarios[0].deficit is None)    or      (pc.scenarios[0].threshold_bonus_share is None)
            or  (pc.scenarios[0].threshold_lr_cutoff is None)):
            pc_missing_test     =  True
    ...
    pc.consistent_pc_latest_param = "PC Parameters upto-date"
