...
def rate_rating_summary(hxd):
    ...
    # if cds.risk_info.new_to_market == "No" and min_conditions:
    if min_conditions:
        # timer.end("prep")
        # timer.start("step 1")
        ####################################################################################
        ...
        benchmark_parameters_df  = hx.params.tb_benchmarkclassparameters  
        ...
        currency_rates_df   = hx.params.tb_currencyrates
        ...
        port_chg_df         = hx.params.tb_portfoliochange
        ...
        ####################################################################################
        #benchmark - load, filter and check available
        rate_chg_df         = hx.params.tb_ratechange
        ...
        ####################################################################################
        #benchmark - load, filter and check available
        infl_chg_df         = hx.params.tb_inflationchange
        ...
        if cds.risk_info.tri_modelling_required == "Yes":
            ...
            bench_all_class_idf_df      = hx.params.tb_afbdevelopmentpatternsoc if cds.triangle_projection.benchmark_use_occurrence else hx.params.tb_afbdevelopmentpatterns
        ...
        all_class_method_att_cred_df                = hx.params.tb_attritionalcredibility
        ...
        single_value_df = hx.params.tb_singlevalues
        ...
        gg_premium_sett             = cds.layers[0].quoted_premium_100pct/1000
        ...
        large_credibility_df = hx.params.tb_largecredibility                                                    #notice we use experience_weight from "LARGE_credibility_df" for both attritional (here) and large (further down)
        ...
        # timer.start("step 15")
        ####################################################################################
        ### 15) Calculate Attritional Loss Load Metrics including Scenarios              ###
        ####################################################################################
        ...
        ## final uw ulr allowing for overrides - notice that the test point is always the INPUT field att.gg_pre_uw_adj.ulr_uw_override
        if      pd.isnull(att.gg_pre_uw_adj.ulr_uw_override):   att.gg_pre_uw_adj.ulr_final_uw  = att.gg_pre_uw_adj.ulr_selected_ol
        elif    (att.gg_pre_uw_adj.ulr_uw_override <0)      :   att.gg_pre_uw_adj.ulr_final_uw  = att.gg_pre_uw_adj.ulr_selected_ol
        else                                                :   att.gg_pre_uw_adj.ulr_final_uw  = att.gg_pre_uw_adj.ulr_uw_override
        ...
        att.gn_pre_uw_adj.ulr_final_uw                      = att.gg_pre_uw_adj.ulr_final_uw                             /  (1 -    total_deductions )
        ...
        # assigning to the actuarial output lr where an input exists - always conditioning on gg_pre_uw_adj
        if pd.isnull(att.gg_pre_uw_adj.ulr_actuarial_override) == False:   att.gn_pre_uw_adj.ulr_actuarial_override_output  = att.gn_pre_uw_adj.ulr_actuarial_override
        ...
        att.gg_pst_uw_adj.ulr_selected_ol                   = att.gg_pre_uw_adj.ulr_selected_ol                         *   (1 +    (att.uw_adjustment or 0))
        att.gg_pst_uw_adj.ulr_uw_override                   = (att.gg_pre_uw_adj.ulr_uw_override or 0)   
        
        if   pd.isnull(att.gg_pre_uw_adj.ulr_uw_override): att.gg_pst_uw_adj.ulr_final_uw   = (att.gg_pst_uw_adj.ulr_selected_ol)
        elif      (att.gg_pre_uw_adj.ulr_uw_override <0) : att.gg_pst_uw_adj.ulr_final_uw   = (att.gg_pst_uw_adj.ulr_selected_ol)
        ...
        att.gn_pst_uw_adj.ulr_uw_override                   = att.gg_pst_uw_adj.ulr_uw_override                          /  (1 -    total_deductions)
        ...
        # timer.start("step 16")
        ####################################################################################
        ### 16) Calculate Large Loss Load Metrics                                        ###
        ####################################################################################
        ...
        ## assigning to equivalent gross gross, post uw adjustments elements of hxd
        lrg.gg_pst_uw_adj.ulr_selected_ol           = lrg.gg_pre_uw_adj.ulr_selected_ol             *   (1 +    (lrg.uw_adjustment or 0))
        if lrg.uw_override: 
        ...
        # timer.start("step 17")
        ####################################################################################
        ### 17) Calculate Cat Loss Load Metrics                                          ###
        ####################################################################################

        ## getting experience weight
        cat_credibility_df = hx.params.tb_catcredibility
        ...
        cat.gg_pst_uw_adj.ulr_selected_ol           = cat.gg_pre_uw_adj.ulr_selected_ol             *  (1 +    (cat.uw_adjustment or 0))
        if cat.uw_override: 
        ...
        ## assigning to equivalent gross net, post uw adjustments elements of hxd
        cat.gn_pst_uw_adj.ulr_selected_ol_exc_nml   = cat.gg_pst_uw_adj.ulr_selected_ol_exc_nml     /  (1 -    total_deductions)
        cat.gn_pst_uw_adj.ulr_selected_ol_nml       = cat.gg_pst_uw_adj.ulr_selected_ol_nml         /  (1 -    total_deductions)
        cat.gn_pst_uw_adj.ulr_selected_ol           = cat.gg_pst_uw_adj.ulr_selected_ol             /  (1 -    total_deductions)
        if cat.uw_override: 
            cat.gn_pst_uw_adj.ulr_uw_override       = 0  
        ...
        tech.amount_pc_100         = ( cds.profit_commission.scenarios[0].result_exp_pc_payable_override 
                                        or cds.profit_commission.scenarios[0].result_exp_pc_payable
                                        or 0 )                                                              #necessary to calculate here and not use selected due to order of calculations
        tech.amount_pc_afb         = tech.amount_pc_100 * (cds.layers[0].written_line or 0)
        gg_prem                    = ( cds.layers[0].quoted_premium_100pct or 0 )
        tech.percent_pc            = 0 if ( gg_prem == 0 ) else tech.amount_pc_100 / gg_prem
        percent_pc                 = tech.percent_pc
        ...
        tech.gg_premium_quoted_100_pct        = (cds.layers[0].quoted_premium_100pct or 0)
        ...
        tech.gg_premium_quoted_afb           = tech.gg_premium_quoted_100_pct * (cds.layers[0].written_line or 0)
        ...
        tot.gg_pre_uw_adj.ulr_priced_final          = att.gg_pre_uw_adj.ulr_final  +   lrg.gg_pre_uw_adj.ulr_final   +   cat.gg_pre_uw_adj.ulr_final 


        # calculate total GROSS NET loss ratios - priced / benchmark / plan / prior - BEFORE UW ADJ
        tot.gn_pre_uw_adj.ulr_priced_final_exc_pc = att.gn_pre_uw_adj.ulr_final  +   lrg.gn_pre_uw_adj.ulr_final   +   cat.gn_pre_uw_adj.ulr_final
        ...
        tech.amts_pre_uw_adj.losses_att_afb  = tech.gg_premium_quoted_afb    *   att.gg_pre_uw_adj.ulr_final
        tech.amts_pre_uw_adj.losses_lrg_afb  = tech.gg_premium_quoted_afb    *   lrg.gg_pre_uw_adj.ulr_final
        tech.amts_pre_uw_adj.losses_cat_afb  = tech.gg_premium_quoted_afb    *   cat.gg_pre_uw_adj.ulr_final
        tech.amts_pre_uw_adj.losses_tot_afb  = (tech.amts_pre_uw_adj.losses_att_afb
                                                   +   tech.amts_pre_uw_adj.losses_lrg_afb
                                                   +   tech.amts_pre_uw_adj.losses_cat_afb)
        ...
        tech.amts_pre_uw_adj.reinsurance_afb  = 0 if tot.gn_pre_uw_adj.ulr_priced_final_inc_pc==0 else (
                                                         tech.amts_pre_uw_adj.losses_tot_afb
                                                            *   (percent_ri_premium - percent_ri_recovery) 
                                                            /    tot.gn_pre_uw_adj.ulr_priced_final_inc_pc)
        ...
        tech.amts_pre_uw_adj.aqn_comm_afb          = (tech.amts_pre_uw_adj.gn_premium_tech_inc_pc_afb
                                                           * (cds.layers[0].commission  or 0)
                                                              / (1 - total_deductions))

        tech.amts_pre_uw_adj.aqn_brok_afb          = (tech.amts_pre_uw_adj.gn_premium_tech_inc_pc_afb
                                                           * (cds.layers[0].brokerage  or 0)
                                                                / (1 - total_deductions))
                                                            
        tech.amts_pre_uw_adj.aqn_iptax_afb         = (tech.amts_pre_uw_adj.gn_premium_tech_inc_pc_afb
                                                           * (cds.layers[0].ipt  or 0)
                                                                / (1 - total_deductions))

        tech.amts_pre_uw_adj.aqn_total_exc_pc_afb  = (tech.amts_pre_uw_adj.aqn_comm_afb
        ...
        tech.amts_pre_uw_adj.gn_premium_tech_exc_pc_afb    = (tech.amts_pre_uw_adj.gn_premium_tech_inc_pc_afb 
                                                                    + tech.amts_pre_uw_adj.aqn_pc_afb)
        ...
        tech.amts_pre_uw_adj.gn_premium_tech_inc_pc_100_pct= ( tech.amts_pre_uw_adj.gn_premium_tech_inc_pc_afb
                                                                     / (cds.layers[0].written_line or 1 ))
        ...
        tech.amts_pst_uw_adj.reinsurance_afb  = 0 if tot.gn_pst_uw_adj.ulr_priced_final_inc_pc==0 else (
                                                         tech.amts_pst_uw_adj.losses_tot_afb
                                                            *   (percent_ri_premium - percent_ri_recovery) 
                                                            /    tot.gn_pst_uw_adj.ulr_priced_final_inc_pc)

        tech.amts_pst_uw_adj.profit_req_afb   = 0 if tot.gn_pst_uw_adj.ulr_priced_final_inc_pc==0 else (
                                                         tech.amts_pst_uw_adj.losses_tot_afb  
                                                            *   percent_roc 
                                                            *   percent_capital_req
                                                            /   tot.gn_pst_uw_adj.ulr_priced_final_inc_pc )

        tech.amts_pst_uw_adj.expense_afb      =(tech.amts_pst_uw_adj.losses_tot_afb  *   percent_expense
                                                                                     /   tot.gn_pst_uw_adj.ulr_priced_final_inc_pc ) 
        tech.amts_pst_uw_adj.investment_afb   =(tech.amts_pst_uw_adj.losses_tot_afb  *   percent_invest_inc
                                                                                     /   tot.gn_pst_uw_adj.ulr_priced_final_inc_pc ) 
        ...
        # calculate amounts associated to acquisition costs PART 1 - 
        tech.amts_pst_uw_adj.aqn_comm_afb          = (tech.amts_pst_uw_adj.gn_premium_tech_inc_pc_afb
                                                           * (cds.layers[0].commission  or 0)
                                                              / (1 - total_deductions))

        tech.amts_pst_uw_adj.aqn_brok_afb          = (tech.amts_pst_uw_adj.gn_premium_tech_inc_pc_afb
                                                           * (cds.layers[0].brokerage  or 0)
                                                                / (1 - total_deductions))
                                                            
        tech.amts_pst_uw_adj.aqn_iptax_afb         = (tech.amts_pst_uw_adj.gn_premium_tech_inc_pc_afb
                                                           * (cds.layers[0].ipt  or 0)
                                                                / (1 - total_deductions))

        tech.amts_pst_uw_adj.aqn_total_exc_pc_afb  = (tech.amts_pst_uw_adj.aqn_comm_afb
                                                                +    tech.amts_pst_uw_adj.aqn_brok_afb  
                                                                +   tech.amts_pst_uw_adj.aqn_iptax_afb)
        
                # note there is a slight inconsistency in the formula below - the PC is treated as multiplicative not additive to the other deductions.
                # however this is the way it is coded in the prior model - flagging it for review at day 2 - this logic has been carried through into the benchmark premium calc
        tech.amts_pst_uw_adj.aqn_pc_afb        = (  (tech.amts_pst_uw_adj.gn_premium_tech_inc_pc_afb
                                                              +   tech.amts_pst_uw_adj.aqn_total_exc_pc_afb)
                                                         * (1 / ( 1  - percent_pc )    -   1 ))    

        tech.amts_pst_uw_adj.aqn_total_inc_pc_afb  = tech.amts_pst_uw_adj.aqn_total_exc_pc_afb   +   tech.amts_pst_uw_adj.aqn_pc_afb


        # calculate technical premium PART 2
        tech.amts_pst_uw_adj.gn_premium_tech_exc_pc_afb    = (tech.amts_pst_uw_adj.gn_premium_tech_inc_pc_afb 
                                                                    + tech.amts_pst_uw_adj.aqn_pc_afb)
        ...
        cds.rating_summary.standard_deviation.attritional   = 0 if np.isnan(sd_att) else   sd_att
        cds.rating_summary.standard_deviation.large         = 0 if np.isnan(sd_lrg) else   sd_lrg
        cds.rating_summary.standard_deviation.catastrophe   = 0 if np.isnan(sd_cat) else   sd_cat

        ####################################################################################
        ### 24) Chart Data - loss ratios                                                 ###
        ####################################################################################

        cds.rating_summary.chart_show_basis_gg = True if cds.rating_summary.chart_basis == "Gross Gross" else False
        cds.rating_summary.chart_show_basis_gn = not cds.rating_summary.chart_show_basis_gg
    ...
    #                                ,"display_cat_chainladder_lr"  #not passing so it remains null/blank
        ...
        write_pd_to_hxd(rating_df,  cds.rating_summary.detail_by_year,  output_columns_str + output_columns_date + output_columns_flt + output_columns_bool)
        ...
        layer.benchmark_premium            = tech.amts_pst_uw_adj.gg_premium_tech_afb 
        layer.bpi                          = kpi.pst_uw_adj.bpi
        layer.bpi_pre_uw_adj               = kpi.pre_uw_adj.bpi 
        layer.technical_premium            = tech.amts_pst_uw_adj.gg_premium_tech_afb
        layer.technical_premium_pre_uw_adj = tech.amts_pre_uw_adj.gg_premium_tech_afb
        layer.technical_premium_net        = tech.amts_pst_uw_adj.gn_premium_tech_inc_pc_afb   # this is gross net, where we also net off PC
        layer.tpi                          = kpi.pst_uw_adj.tpi
        layer.tpi_pre_uw_adj               = kpi.pre_uw_adj.tpi 
        layer.pflr_att                     = (att.gg_pst_uw_adj.ulr_final + lrg.gg_pst_uw_adj.ulr_final)    / (1 - percent_pc - total_deductions)
        layer.pflr_cat                     = (cat.gg_pst_uw_adj.ulr_final)                                  / (1 - percent_pc - total_deductions)
        layer.pflr                         = layer.pflr_att + layer.pflr_cat 
        layer.roc                          = kpi.pst_uw_adj.roc
        layer.uw_adj_impact                = 0 if tech.amts_pst_uw_adj.losses_tot_afb == 0 else (tech.amts_pre_uw_adj.losses_tot_afb  /  tech.amts_pst_uw_adj.losses_tot_afb) -1
        layer.expected_loss_cost           = tech.amts_pst_uw_adj.losses_tot_afb
        layer.expected_loss_cost_pre_uw_adj= tech.amts_pre_uw_adj.losses_tot_afb
    # not performing any calculatios
    cds.layers[0].rate_change.rate_change.uw_selected           = cds.rating_summary.detail_by_year[-1].rate_chg_override if cds.rating_summary.detail_by_year[-1].rate_chg_override is not None else cds.rating_summary.detail_by_year[-1].rate_chg_suggested
    pass
