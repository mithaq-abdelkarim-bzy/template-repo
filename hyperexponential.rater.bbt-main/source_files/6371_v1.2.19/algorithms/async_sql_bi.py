...
def bi_facility_fetch(hxd,progress):
    ...
    if main_facility_df.empty:
        ...
        #cds.layers[0].quoted_premium_100pct        =  ##done as part of loading remaining data inc. claims as needs fx conversion
...
def bi_clm_and_mvmt_fetch(hxd,progress,include_triangles):
    ...
    if claim_summary_detail_df.empty:
        # do nothing
        cds.bi_data.fetch_clm_and_mvmt_detail_task_status = "Claim Information not found for selected references"
        return
    ...
    currency_rates_bi_df= hx.params.tb_currencyratebicurrent
    ...
    spot_fx_df                          = hx.params.tb_currencyratebicurrent
...
def update_premium_table (hxd,progress):
    ...
    #########################################################
    spot_fx_df                          = hx.params.tb_currencyratebicurrent
    policy_df                           = pd.merge(policy_df,spot_fx_df[['Currency','ExchangeRate']], left_on='settlement_currency', right_on='Currency', how='left')
