...
def ratio(a, b, if_undefined=0):    
    ...
    # Treat None as undefined (prevents pd.Series(None) -> empty Series)
...
def safe_sum(x, default=0.0):
    """
    Sum that is safe for pandas Series, numpy arrays, scalars, or None.
    Always returns a scalar float.
    """
    if x is None:
        return default
...
# uses alternate version of look_up above
# # Convert to USD
...
def yearfrac_basis0(start, end):

    sta_scalar = not isinstance(start, (pd.Series, pd.Index, np.ndarray, list, tuple))
    end_scalar = not isinstance(end,   (pd.Series, pd.Index, np.ndarray, list, tuple))
    was_scalar = sta_scalar and end_scalar

    if was_scalar:
        start   = pd.Series([pd.Timestamp(start)])
        end     = pd.Series([pd.Timestamp(end)])
    else:
        start   = pd.Series(pd.to_datetime(start))
        end     = pd.Series(pd.to_datetime(end))
    ...
    d1_adj = np.where(d1 == 31, 30, d1)
    d2_adj = np.where((d2 == 31) & (d1_adj >= 30), 30, d2)

    result = ( 360 * (y2 - y1)   +    30 * (m2 - m1)   +    (d2_adj - d1_adj)) / 360

    result = pd.Series(result, index=start.index).fillna(0)

    return result.iloc[0] if was_scalar else result
...
def get_fx_rate(df, ccy):
    # scalar case
    if not isinstance(ccy, (pd.Series, pd.Index, np.ndarray, list, tuple)):
        row = df.loc[df['ccy'] == ccy, 'fx_rate']
        return row.iat[0] if not row.empty else 1

    # vector case
    fx_map = df.set_index('ccy')['fx_rate']
    fx = pd.Series(ccy).map(fx_map).fillna(1)

    return fx



def stratified_uniform(n, rng, shuffle) -> np.ndarray:
    if n <= 0:
        return np.empty(0)
