...
from algorithms.rate_utilities import policy_term, nan_to_none
...
def task_rarc_layer(hxd, progress):
    ...
    """
    from algorithms.tasks import task_simulation    # JB ADDED
    ...
    rc = RateChangeLib(
        ...
        async_tasks=[task_simulation],  # Pass the actual tasks name as a variable, not a string
    ...
    # remove the nan by none to avoid issues in writing in hxd
    # rarc_df = rarc_df.where(pd.notnull(rarc_df), None)

    # Apply to your rarc_list
    rarc_df = nan_to_none(rarc_df)
    rarc_list = nan_to_none(rarc_list)
