...
def sch_rate_change(cds):
    ...
    for change in rarc_changes.keys():
        ...
        cds.override_node_properties(f"cds/layers/rate_change/{change}/uw_override",     {"view": {"label": "UW Override", "format": percent_format(1),"read_only": change == "rate_change"}, "async_output": [] if change == "rate_change" else ["task_start_renewal"]})
        cds.override_node_properties(f"cds/layers/rate_change/{change}/final",           {"view": {"label": "Final",       "format": percent_format(1)}} )
        cds.override_node_properties(f"cds/layers/rate_change/{change}/comments",{"async_output": ["task_start_renewal"]} ) # added this to clear out the input on start of renewal.
    ...
    cds.override_node_properties(
        ...
        {  
            ...
            "async_output": [{"task": "task_start_renewal", "reset": False}],
            "options": [*range(1,max_layers+1)],
    })

    cds.extend_node_rater_defined("cds/layers/rate_change", {
        "risk_adjusted_rate_change": hx.Float(mode="output", view={"label": "Final Risk Adjusted Rate Change", "format": percent_format()}), # EDIT v0.3.0 removed since it has moved to CDS 1.3.0 - spoke to IJ - reinstate
        ...
        **{
            cvg: hx.Structure(view=cvg_view, children={
                **{change: hx.Structure(view=rc_view, children={
                    ...
                    "uw_selected": hx.Float(mode="output" if change=="rate_change" else "override", async_output=[{"task": rarc_task_name, "reset": False},"task_start_renewal"], view={"label": "UW Selected", "format": percent_format(1)}),
                    "uw_override": hx.Float(mode="input", default=None, optionality="optional", async_output=[{"task": rarc_task_name, "reset": False}, "task_start_renewal"],view={"label": "UW Override %", "format": {"output": "percent", "mantissa": 1}}),
                    "final": hx.Float(mode="output", view={"label": "Final %", "format": {"output": "percent", "mantissa": 1}}),
                    "comments": hx.Str(mode="input", default=None, optionality="optional", view={"label": "Comment"}, async_output=[{"task": "task_start_renewal", "reset": False}])
    ...
    cds.extend_node_rater_defined("cds", {
        "rate_change": hx.Structure(children={
            **{f"show_layer_{index}": hx.Bool(mode="output") for index in range(1,6+1)},
            "expiring_policy_option_id": hx.Int(mode="override", async_input=rarc_tasks_names, async_output=["task_start_renewal","sync_expiring_ids"],view={"label": "Expiring Policy Option", "format": {"thousandSeparated": False}}),
    ...
    cds.override_node_properties("cds/currencies/source_currency", {'default': "USD","async_input": [rarc_task_name],"async_output": [{"task": rarc_task_name, "reset": False}]})
