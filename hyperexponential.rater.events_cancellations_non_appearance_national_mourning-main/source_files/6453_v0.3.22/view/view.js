
// v0.5.0
/* eslint-disable */
import * as HX from "hx-model-components";
import { vw_risk_information } from "view/vw_risk_information";
import { vw_exposure_rating_event_cancellation } from "view/vw_exposure_rating_event_cancellation";
import { vw_exposure_rating_national_mourning } from "view/vw_exposure_rating_national_mourning";
import { vw_exposure_rating_non_appearance } from "view/vw_exposure_rating_non_appearance";
import { vw_actuarial } from "view/vw_actuarial";
import { vw_experience_rating } from "view/vw_experience_rating";

import { vw_rating_summary } from "view/vw_rating_summary";
import { vw_rating_summary_case_priced } from "view/vw_rating_summary";
import { vw_rate_change } from "view/vw_rate_change";
import { vw_rationale } from "view/vw_rationale";
import { vw_landing_page } from "view/vw_landing_page";
import { vw_standard_kpi } from "view/vw_standard_kpi";
import { vw_standard_kpi_case_priced } from "view/vw_standard_kpi";
import { vw_bug_report } from "libraries/email_notification/view/vw_bug_report";
import SchemaViewPage from "libraries/schema_viewer/view/schema_view";
import timerProfiling from "libraries/model_profiler/view/timerProfilingPage";





function HXModel() {
  const scale = 1
  return (
    <HX.Root>
      {vw_landing_page(scale)}
      {vw_risk_information(scale)}
      {vw_exposure_rating_event_cancellation(scale)}
      {vw_exposure_rating_national_mourning(scale)}
      {vw_exposure_rating_non_appearance(scale)}

      {vw_experience_rating(scale)}
      {vw_rating_summary(scale)}
      {vw_rating_summary_case_priced(scale)}
      {vw_rate_change(scale)}
      {vw_rationale(scale)}
      {vw_standard_kpi(scale)}
      {vw_standard_kpi_case_priced(scale)}
      {vw_actuarial(scale)}
      {vw_bug_report(scale)}
      {SchemaViewPage(scale)}
      {timerProfiling()}
      {/* <HX.Page title="Something Is Broken" fullWidth={false} viewScale={scale} shownBy="model_state/show_after_landing_page">
        <HX.Section title="Bug Report">
          <HX.Notes field="bug_report_email" />
        </HX.Section>
      </HX.Page> */}
    </HX.Root >
  );
}

export default HXModel;



