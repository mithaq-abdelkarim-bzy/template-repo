...
#' Author: Mark Fleet
#' Date: 29/10/2025
#' Description: Update PAS references
#' 
#'
#' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ '#
    ...
    try: 
        ...
        if result_json.status_code != 200:
            if 'title' in result_json.json():
                ...
                return {'status' : "failed", "error_step": step_desc, "error_message": result_json.status_code, "error_detail": result_json.json()["title"]}
            else:
...
def datafix(output):
    ...
    pas_references = row["pas_references"].split(',')
    ...
    # Set URLs
    policy_url = f"{config.base_url}/policies/{policy_id}"

    # Set data
    json_dat_policy = {
    "pas_references" : pas_references,
    }

    #Patch
    patch_result = api_request_retry("patch", policy_url, headers = headers_policy, json = json_dat_policy)
    failure = detect_api_failure(patch_result, "patch policy")
    if failure != None:
