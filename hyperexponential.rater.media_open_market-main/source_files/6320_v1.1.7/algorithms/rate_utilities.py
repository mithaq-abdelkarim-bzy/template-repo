...
# Filter and find last row lookup
def find_last_row(parameter_table, column_name, lookup_value):
    '''
    Find last row for lookup value in relevant column. 
    Note, column_name and return_column are strings.
    '''
    filtered = parameter_table[parameter_table[column_name] <= lookup_value]
...
# Lookup in parameter table, return relevant value
def parameter_lookup(parameter_table, lookup_column, lookup_value, return_column, if_not_found):
    value = parameter_table[parameter_table[lookup_column] == lookup_value][return_column].iloc[0] if lookup_value is not None else if_not_found
    return value