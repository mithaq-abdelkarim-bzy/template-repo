...
def pd_df_from_hx_list(hx_list, columns: Optional[List[str]] = None):
    '''
...
def write_pd_to_hxd(df, list_node, output_cols_to_write, override_cols_to_write = []):
    ...
    for i in range(df.shape[0]):
        for col in output_cols_to_write:
            rsetattr(list_node[i], col, dictionary[col][i])
...
def ratio(a, b, if_undefined=0):
    ...
    Handles division by zero by returning 'if_undefined'.
    Supports scalar, Series, and NumPy array inputs.
    """
    ...
    # Handle broadcasting if one Series has length 1
    if len(a) == 1 and len(b) > 1:
        a = pd.Series([a.iloc[0]] * len(b))
    elif len(b) == 1 and len(a) > 1:
        b = pd.Series([b.iloc[0]] * len(a))
