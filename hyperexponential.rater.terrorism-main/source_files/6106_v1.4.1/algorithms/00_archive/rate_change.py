...
class RateChange:
    ...
    def calculate_repriced_values(
        ...
        for i, dict_items in enumerate(self.buckets.items()):
            ...
            if len(common_items) > 0: # Only update if there is at least one common item
                print(f"Updating inputs for: {bucket_name}")
                for path in paths:
                    # self._update_expiring_hxd(self.rc_hxds[bucket_name], path, list_excluded_from_padding=split_list_path)
