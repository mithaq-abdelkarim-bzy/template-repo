...
from algorithms.rate_change.rate_rate_change_global_variables import RATE_CHANGE_BUCKETS_STEER, RATE_CHANGE_BUCKETS_HEALTHCARE_CAT, RATE_CHANGE_BUCKETS_CLASH, EXP_INPUTS_IN_CCY_STEER, EXP_INPUTS_IN_CCY_HEALTHCARE_CAT, EXP_INPUTS_IN_CCY_CLASH
...
def rate_rate_change(hxd):
    """
    Assign rate change renewing and expiring premium, calcuflate the rarc and set some validation on layer and premium
    ...
    for idx, layer in enumerate(layers): 
        ...
        for item in ["exposure_change", "risk_characteristics_change", "deductible_change", "limit_change", "terms_conditions_change", "other_change"]:
            ...
            rate_change_multiplier = rate_change_multiplier * (rc_vbl.final or 0)
            ...
            if rc_vbl.uw_override is not None and rc_vbl.comments is None:
                hx.errors.validation(f"Rate Change - {(title_rc(item))} has been overridden. Please provide comments")
        ...
        layer.rate_change.rate_change.final = final_rarc 
        ...
        layer.rate_change.rate_change_multiplier = rate_change_multiplier
        layer.rate_change.implied_renewable_rate = layer.rate_change.rate.expiring * rate_change_multiplier
        layer.rate_change.implied_renewable_premium = layer.rate_change.premium.line_100pct.annualised.expiring * rate_change_multiplier
