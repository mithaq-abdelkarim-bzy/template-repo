...
def rate_steer_triangle_count_projection(hxd):
    cds   = hxd.cds
    tri   = hxd.cds.steer.experience_rating.layers.fgu.claim_count    
    ...
    short_or_long_tailed    = "LT" 

    #######################################################################################
    ### 1) BUILDING TRIANGLE WHERE we have a complete triangle
    #######################################################################################
    ...
    if ( len(processed_claims)!=0 or (override_triangle_active and override_triangle_valid) ):
        ...
        ### Build Tri_3 (selected): if triangle override has been selected set tri_3 equal to override otherwise use raw data
        ...
        tri_3.selected_average_option = mapping_option_to_node[tri.selected_average_option_input]

        #############################################
        ## 1e) assign exclusions triangle
        #############################################

        # Address renewal (hx_core inception year + 1 on renewal causing list index out of range error)
        try:
    ...
    if len(processed_claims)!=0 or ((override_triangle_active and override_triangle_valid)) and tri_3_status:
        ...
        # Default value
        tri.experience_weight.selected  = 1
    ...
    #######################################################################################
        ...
        ###################################
        # # TODO - remove if not used in steer
        ...
        # TODO IH - Add assignement to tri_3 for override and selected
        # try:
        #     for k in range(tri_3.dev_period_count-1):
        #         if not pd.isna(custom_idf_df.experience_override[k]):
        #             tri_3.idf_overrides[k] = custom_idf_df.experience_override[k]
    ...
    write_pd_to_hxd(custom_idf_df.fillna(1), hxd.cds.steer.experience_rating.layers.fgu.claim_count.incremental_dev_factor, output_columns)
    ...
    # writign to hxd - enhancement here would be to not fill show_row with 1???  
    write_pd_to_hxd(custom_idf_df.fillna(1), hxd.cds.steer.experience_rating.layers.fgu.claim_count.percents_ultimate, output_columns)
