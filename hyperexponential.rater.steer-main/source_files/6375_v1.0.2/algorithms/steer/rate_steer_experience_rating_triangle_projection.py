...
def rate_steer_triangle_projection(hxd):
    """ 
    - Populate Raw Data Triangle with incurred or paid triangle from processed claims. 
    - Populate Selected Triangle (Used for analysis) from Raw Data, Override Triangle. Takes into account of Override of Development Factors.
    - Populate Result Triangle (fully projected, used for Charts)
    ...
    for layer_name in layer_list:
        ...
        override_triangle_valid  = (((tri_2.origin_period_count or 0) == min(const.experience_rating_max_years, max(1,tri.override_triangle_years)))
        ...
        short_or_long_tailed    = "LT" # Not used in the Excel Model, but part of the Params table

        #######################################################################################
        ### 1) BUILDING TRIANGLE from claims data
        #######################################################################################
        ...
        if len(processed_claims)!=0:
            ...
            # initial settings
            # last_origin_year            = int(lvy)
            # first_origin_year           = int(fvy)
            ...
            # extracted_date              = (hxd.cds.steer.experience_rating.layers.fgu.triangle_projection.override_triangle_date or incept_date)
            ...
            # for row in processed_claims_df.index:
            #     origin_period   = datetime.date(int(processed_claims_df.yoa[row]),  first_origin_month,  first_origin_day)
            #     dev_period      = ( processed_claims_df.mvmt_yr[row] -1 ) *12  + mths_first_dev_period                      #first year is 1 so net of 1 year convert to months and then add back on the starting number of months
            #     value           = processed_claims_df.incurredmvmt_100_sett_fx[row]
        ...
        tri_source = tri_1a if tri.tri_1_basis == "Incurred" else tri_1b

        # assigning triangle dimension values
        tri_1.origin_period_count   = tri_source.origin_period_count
        tri_1.dev_period_increment  = tri_source.dev_period_increment
        tri_1.dev_period_count      = tri_source.dev_period_count
        tri_1.first_dev_period      = tri_source.first_dev_period

        # Can't use df => use loop to assign triangles
        for k in range(tri_source.origin_period_count):
            tri_1.origin_periods[k] = tri_source.origin_periods[k]

        # Except address renewal (hx_core inception year + 1 on renewal causing list index out of range error)
        try:
            for k in range(tri_source.origin_period_count):
                for j in range(tri_source.dev_period_count):
                    tri_1.incremental_data[k,j]  = tri_source.incremental_data[k,j] 
                    
        except:
            for k in range(tri_source.origin_period_count):
                for j in range(tri_source.dev_period_count - 1):
                    tri_1.incremental_data[k,j]  = tri_source.incremental_data[k,j] 
        ...
        if ( len(processed_claims)!=0 or (override_triangle_active and override_triangle_valid) ):
            
            ### Build Tri_3 (selected): if triangle override has been selected set tri_3 equal to override otherwise use tri_1 raw data
            if (override_triangle_active and override_triangle_valid):
                ...
                #############################################
                ## 1d) BEGIN Build Tri_3 (selected) from Raw Data
                #############################################
                # assigning triangle dimension values
            ...
            mapping_option_to_node = {"5-year":"vw_5", "7-year":"vw_7", "15-year":"vw_15","all-year":"vw_all"}
            tri_3.selected_average_option = mapping_option_to_node[tri.selected_average_option_input] # from manual entry
        ...
        if len(processed_claims)!=0 or ((override_triangle_active and override_triangle_valid)) and tri_3_status:

            ###############################################################################
            ### 2a) Triangles Available - BEGIN TAIL FACTOR - Using Benchmark Tail from parameters
            ###############################################################################
            ...
            index = last_dev_period - 2
            if 0 <= index < len(tri_3.idfs_calculated):
                last_dev_factor = max(1, tri_3.idfs_calculated[index])
            else:
                last_dev_factor = 1
            ...
            # TODO confirm if should have a short tail long tail
            tf_rows_df = tf_df[ (tf_df.basis=="cumulative")   &  (tf_df.tail_df <= last_dev_factor) ].sort_values(by=['tail_df'])
            ...
            num_years_data                  = tri_3.origin_period_count                                                             # could consider if some years are nil, but old algorithm didnt
            ...
            weight_conditions               = (weight_df.num_yr_data<=num_years_data)  &  (weight_df.tail_code == "LT")  &  (weight_df.average_total_claims <= average_incurred)
            ...
            # Commented until the transient Library is upgraded to address the Triangle Component
            ...
            tri.tail_factor.experience_default  = 1 if tf_rows_df.empty else tf_rows_df.incurred_dev_factor.iat[-1] # as per original tail factor calculation
            
            # Commented until the transient Library is upgraded to address the Triangle Component
            # tri.tail_factor.experience_selected = tri_3.tail_factor = tri.tail_factor.experience_default if ( pd.isna(tri.tail_factor.experience_override)
            #                                                                                     or tri.tail_factor.experience_override <= 0) else  tri.tail_factor.experience_override
            tri.tail_factor.experience_selected =  tri_3.tail_factor = tri.tail_factor.experience_selected_set_in_task 


            # Add assignement to tri_3 for override and selected
            # try:
            #     for k in range(tri_3.dev_period_count-1):
            #         if not pd.isna(custom_idf_df.experience_override[k]):
            #             tri_3.idf_overrides[k] = custom_idf_df.experience_override[k]
                    
            # except:
            #     for k in range(tri_3.dev_period_count-2):
            #         if not pd.isna(custom_idf_df.experience_override[k]):
            #             tri_3.idf_overrides[k] = custom_idf_df.experience_override[k]
            ...
            try:
                for k in range(tri_3.dev_period_count-1):
                    if k < const.experience_rating_max_years:
                        if not pd.isna(custom_idf_df.experience_override[k]):
                            tri_3.idf_overrides[k] = custom_idf_df.experience_override[k]
                        #
            except:
        ...
        #######################################################################################
            ...
            # extracted_date              = (hxd.cds.steer.experience_rating.layers.fgu.triangle_projection.override_triangle_date or incept_date)
            extracted_date              = (getattr(hxd.cds.steer.experience_rating.layers,layer_name).triangle_projection.override_triangle_date or incept_date)
            last_mvmt_date              = min(incept_date,extracted_date)
            delta_date                  = relativedelta(last_mvmt_date, first_origin_date)
            mths_total                  = delta_date.years * 12 + delta_date.months
            mths_first_dev_period       = mths_total % 12  # modulo or remainder
            ...
            # # Set the benchmark override name3
            tri.benchmark_name.default      = cds.standard_fields.benchmark_class
            tri.benchmark_name.selected     = tri.benchmark_name.override or tri.benchmark_name.default
            ...
            # Assignement to tri_3 for override and selected
            # try:
            #     for k in range(tri_3.dev_period_count-1):
            #         if not pd.isna(custom_idf_df.experience_override[k]):
            #             tri_3.idf_overrides[k] = custom_idf_df.experience_override[k]
            ...
            # except:
            #     for k in range(tri_3.dev_period_count-2):
            #     # for k in range(max(24,tri_3.dev_period_count-2)):    
            #         if not pd.isna(custom_idf_df.experience_override[k]):
            #             tri_3.idf_overrides[k] = custom_idf_df.experience_override[k]
        ...
        # Condition until transient is upgraded to address the Triangl node
        if tri.tail_factor.experience_selected is not None:
            ...
            tri.tail_factor.blended_selected = 1
        ...
        custom_idf_df.benchmark_default_perc_ult      = custom_idf_df.benchmark_default_perc_ult
        ...
        write_pd_to_hxd(custom_idf_df.fillna(1), getattr(hxd.cds.steer.experience_rating.layers,layer_name).triangle_projection.incremental_dev_factor, output_columns)
        ...
        write_pd_to_hxd(custom_idf_df.fillna(1), getattr(hxd.cds.steer.experience_rating.layers,layer_name).triangle_projection.percents_ultimate, output_columns)
        ...
        if ((len(processed_claims)!=0) or (override_triangle_active and override_triangle_valid) ) and tri_3_status:
            ...
            tri_4.tail_factor             = tri.tail_factor.experience_selected or 1
            ...
            # Address case of renewal (hx_core inception year + 1 on renewal causing list index out of range error)
            try:
                # Assigning Data matrix
                for k in range(tri_3.origin_period_count):
                ...
                for k in range(tri_4.dev_period_count-1):
                    ...
                    tri_4.idf_overrides[k] = tri_3.idfs_selected[k]
            except:
                # Assigning Data matrix
                for k in range(tri_3.origin_period_count):
                ...
                for k in range(tri_4.dev_period_count - 2):
                    # tri_4.idf_overrides[k] = (tri.incremental_dev_factor[k].blended_selected)  if (k < const.experience_rating_max_years)   else 1
                    tri_4.idf_overrides[k] = (tri_3.idfs_selected[k])  if (k < const.experience_rating_max_years)   else 1
