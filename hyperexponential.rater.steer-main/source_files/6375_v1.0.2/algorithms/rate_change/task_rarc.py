...
from algorithms.rate_change.algorithms.rate_change import RateChange as RateChangeLib
...
from algorithms.rate_rate_change import RATE_CHANGE_BUCKETS_STEER, RATE_CHANGE_BUCKETS_HEALTHCARE_CAT, RATE_CHANGE_BUCKETS_CLASH, EXP_INPUTS_IN_CCY_STEER, EXP_INPUTS_IN_CCY_HEALTHCARE_CAT, EXP_INPUTS_IN_CCY_CLASH, define_insured_asset_list_variable # Edit v0.3.0
...
def task_rarc_layer(hxd, progress):
    ...
    """
    from algorithms.tasks import steer_format_raw_data_task, steer_populate_bc_patterns_task

    er = hxd.cds.steer.experience_rating
    ms = hxd.model_state
    is_pure_premium_not_calculated = ms.is_pure_premium_not_calculated

    # has_missing_pure_premiums = hxd.model_state.is_pure_premium_not_calculated 
    # steer_bc_pattern_not_updated = ms.is_steer_experience_rating and (not er.layers.fgu.triangle_projection.has_populated_bc_patterns or er.is_not_experience_selected_updated)
    

    # if is_pure_premium_not_calculated:
    #     # return # exit
    #     hx.errors.fatal("Address issues in Pricing Selection or Burning Cost.")
    ...
    # Get buckets 
    if ms.is_steer:
        buckets = RATE_CHANGE_BUCKETS_STEER
    elif ms.is_healthcare_cat:
        buckets = RATE_CHANGE_BUCKETS_HEALTHCARE_CAT
    elif ms.is_clash:
        buckets = RATE_CHANGE_BUCKETS_CLASH
    ...
    if ms.is_steer:
        expiring_inputs_in_ccy = EXP_INPUTS_IN_CCY_STEER["layers"]
    elif ms.is_healthcare_cat:
        expiring_inputs_in_ccy = EXP_INPUTS_IN_CCY_HEALTHCARE_CAT["layers"]
    elif ms.is_clash:
        expiring_inputs_in_ccy = EXP_INPUTS_IN_CCY_CLASH["layers"]
    ...
    rc = RateChangeLib(
        ...
        async_tasks=[steer_format_raw_data_task,steer_populate_bc_patterns_task],  # Pass the actual tasks name as a variable, not a string
    ...
    # print(rarc_df)
    # print(rarc_list)
    # Remove Nan from rarc_list
    for item in rarc_list:
        for key in ["exposure_change", "risk_characteristics_change", "deductible_change", "limit_change", "terms_conditions_change", "other_change"]:
            if key in item and "model_calculated" in item[key]:
                if math.isnan(item[key]["model_calculated"]):
                    item[key]["model_calculated"] = None

    default_temp_storage = {'quoted_premium': 0, 'benchmark_premium': 0, 'currency': 'USD'}
    ...
    for rarc_layer, hxd_layer in zip(rarc_list, hxd.cds.layers):
        ...
        # hxd_layer.rate_change.temp_storage = np.where(
        #     rarc_layer["temp_storage"].isna() | (rarc_layer["temp_storage"] == None),
        #     default_temp_storage,
        #     rarc_layer["temp_storage"]
        # )
