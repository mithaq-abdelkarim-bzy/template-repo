...
def assign_expiry_nodes_rate_change(rc_source_data, rc_data,expiring_inception_date,expiring_expiry_date): # Edit v0.3.0
    """
    ...
    """
    # Expiring info
    rc_data.expiring_policy_info.expiring_quoted_premium_100 = (rc_source_data["quoted_premium_100"] or 0)   
    rc_data.expiring_policy_info.expiring_quoted_premium = (rc_source_data["quoted_premium"] or 0)

    # annulised specifically for STEER to allow inputs only model to capture annualised premiums 100
    if (rc_source_data["quoted_premium_100"] !=0 or rc_source_data["quoted_premium_100"] is not None) and (rc_source_data["quoted_premium_annual_100"]==0 or rc_source_data["quoted_premium_annual_100"] is None):
        expiring_inception_date_formatted = datetime.strptime(expiring_inception_date, '%Y-%m-%d')
        expiring_expiry_date_formatted = datetime.strptime(expiring_expiry_date, '%Y-%m-%d')
        expiry_policy_term = policy_term(expiring_inception_date_formatted,expiring_expiry_date_formatted)
        rc_data.expiring_policy_info.expiring_quoted_premium_annual_100 = (rc_source_data["quoted_premium_100"] or 0) * expiry_policy_term
        rc_data.expiring_policy_info.expiring_quoted_premium_annual = (rc_source_data["quoted_premium"] or 0) * expiry_policy_term
    else:
        rc_data.expiring_policy_info.expiring_quoted_premium_annual_100 = (rc_source_data["quoted_premium_annual_100"] or 0)
        rc_data.expiring_policy_info.expiring_quoted_premium_annual = (rc_source_data["quoted_premium_annual"] or 0)
      
    # annualised specifically for STEER to allow inputs only model to capture annualised premiums 100
    ...
    rc_data.expiring_policy_info.expiring_rate = rc_source_data["rate"] or 0
    ...
    for idx, layer in enumerate(hxd.cds.layers):
        ...
        if layer.rate_change.renewing_layer == True:
            ...
            assign_expiry_nodes_rate_change(
                ...
                expiring_inception_date = expiring_data["hx_core"]["inception_date"],
                expiring_expiry_date = expiring_data["hx_core"]["expiry_date"]
                )
