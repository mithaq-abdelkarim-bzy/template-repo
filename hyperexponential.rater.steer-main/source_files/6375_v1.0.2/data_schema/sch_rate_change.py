...
def sch_rate_change(cds):
    ...
    for change in rarc_changes.keys():
        # EDIT v0.3.0
        cds.override_node_properties(f"cds/layers/rate_change/{change}/model_calculated",{"view": {"label": "Model", "format": percent_format(2)},"async_output": [] if change == "rate_change" else [rarc_task_name]} )
        cds.override_node_properties(f"cds/layers/rate_change/{change}/uw_selected",{"view": {"label": "UW Selected", "format": percent_format(2)},"async_output": [] if change == "rate_change" else [{"task": rarc_task_name, "reset": False},"start_renewal_task"]} )  # added this to clear out the override on start of renewal but keep the selected value when Calculating Rate change again.
        cds.override_node_properties(f"cds/layers/rate_change/{change}/uw_override",{"async_output": ["start_renewal_task"], "view": {"label": "UW Override", "format": percent_format(2),"read_only": True} if change == "rate_change" else {"label": "UW Override", "format": percent_format(2)}})
        cds.override_node_properties(f"cds/layers/rate_change/{change}/final",{"view": {"label": "Final", "format": percent_format(2)}} )
    ...
    cds.extend_node_rater_defined("cds/layers/rate_change", {
        ...
        "risk_adjusted_rate_change_case_priced": hx.Float(mode="input", optionality='optional', default=None, view={"label": "Final Risk Adjusted Rate Change", "format": percent_format(2), "options": {"read_only": {"read_only": True}}}),
        ...
        **{
            cvg: hx.Structure(view=cvg_view, children={
                **{change: hx.Structure(view=rc_view, children={
                    "model_calculated": hx.Float(mode="output", async_output=[] if change=="rate_change" else [rarc_task_name], view={"label": "Model", "format": percent_format(2)}),
                    "uw_selected": hx.Float(mode="output" if change=="rate_change" else "override", async_output=[{"task": rarc_task_name, "reset": False},"start_renewal_task"], view={"label": "UW Selected", "format": percent_format(2)}),
                    "uw_override": hx.Float(mode="input", default=None, optionality="optional", async_output=[{"task": rarc_task_name, "reset": False}, "start_renewal_task"],view={"label": "UW Override %", "format": {"output": "percent", "mantissa": 2}}),
                    "final": hx.Float(mode="output", view={"label": "Final %", "format": {"output": "percent", "mantissa": 2}}),
                ...
                "rate_change": hx.Structure(view={"label": "Risk Adjusted Rate Change"}, children={
                    ...
                    "uw_override": hx.Float(mode="input", default=None, optionality="optional", view={"label": "UW Override", "format": {"output": "percent", "mantissa": 2}, "read_only": True}),
                    "final": hx.Float(mode="output", view={"label": "Final", "format": {"output": "percent", "mantissa": 2}}),
                }),
                "risk_adjusted_rate_change": hx.Float(mode="output", view={"label": "Final Risk Adjusted Rate Change", "format": percent_format(2)}),
                "risk_adjusted_rate_change_case_priced": hx.Float(mode="input", optionality='optional', default=None, view={"label": "Final Risk Adjusted Rate Change", "format": percent_format(2), "options": {"read_only": {"read_only": True}}}),
                "risk_adjusted_rate_change_gross_for_reporting": hx.Float(mode="output", view={"label": "Risk Adjusted Rate Change (Gross For Reporting)", "format": percent_format(2)}),
                **rc_quoted_premium(),
                **rc_expiring_and_renewal(),
                "expiring_policy_info": hx.Structure(children={**rc_expiring_info()}),
                "error_message": hx.Str(mode="output"),
    ...
    cds.override_node_properties("cds/currencies/source_currency", {"default":"USD","async_input": [rarc_task_name,"steer_format_raw_data_task"],"async_output": [{"task": rarc_task_name, "reset": False}],"options_column":"ccy", "options_table":"table_currency"})
    
def _steer_rate_change():
    node_info_list= [
        ("Rate Change Multiplier",hx.Float,"output",None,utils.percent_format(2),None,None),
        ("Implied Renewable Rate",hx.Float,"output",None,utils.percent_format(2),None,None),
        ("Implied Renewable Premium",hx.Float,"output",None,utils.thousands_format(0),None,None),
        # ("Actual Rate as per slip",hx.Float,"output",None,utils.percent_format(2),None,None),
        # ("Actual Premium as per slip",hx.Float,"output",None,utils.thousands_format(0),None,None),
    ]
