...
const coordPropTypesDualYAxis = {
    series: HX.PropTypes.arrayOfType(
        HX.PropTypes.objectOfType({
            ...
            xAxisTickFormat: HX.PropTypes.string.optional,
            yAxisTickFormat: HX.PropTypes.string.optional,
            y2AxisTickFormat: HX.PropTypes.string.optional,
            // xAxisMin: HX.PropTypes.number.optional,
        })
    ),
};
...
const TwoAxisLineChart = HX.buildCustomComponent({
    ...
    propTypes: {
        ...
        xAxisMin: HX.PropTypes.number.optional,
        xAxisStep: HX.PropTypes.number.optional,
        xAxisTickFormat: HX.PropTypes.string.optional,
        yAxisTickFormat: HX.PropTypes.string.optional,
        y2AxisTickFormat: HX.PropTypes.string.optional,
    },
    ...
    render: (props, data, tools) => {
        ...
        // const xValues = lineData.flatMap(series =>
        //     series.points.map(p => p.x)
        // );
        const xValues = lineData.flatMap(series =>
            series.points.map(p => Number(p.x))
        );

        const xMin = props.xAxisMin !== undefined
            ? props.xAxisMin
            : Math.min(...xValues);

        const xMax = Math.max(...xValues);

        const xStep = props.xAxisStep ?? 1;
                ...
                <Plot
                    ...
                    layout={{
                        ...
                        xaxis: {
                            ...
                            tickmode: "linear",
                            tickformat: props.xAxisTickFormat || '.0%', // ',.1f' for thousand separated with 1 decimal
                            range: [xMin, xMax],
                            // dtick: props.xAxisStep || undefined,
                            dtick: xStep,
                        },
                        yaxis: {
                            title: props.yAxisLabel,
                            tickformat: props.yAxisTickFormat || '.0%',
                        ...
                        yaxis2: {
                            ...
                            overlaying: "y",
                            side: "right",
                            tickformat: props.y2AxisTickFormat || '.0%',
