    ...
    for i in range(df.shape[0]):
        ...
        if override_cols_to_write:
            for col in override_cols_to_write:
                ...
                setattr(getattr(list_node[i], col),"calculated",dictionary[col][i])
...
# Function for approximate lookup, handles errors by returning 0
def approx_lookup(lookup_value, lookup_col, return_col, df, default=0):
    idx = np.searchsorted(df[lookup_col], lookup_value, side='right') - 1
    if idx < 0:
        return default
    return df[return_col].iloc[idx]
