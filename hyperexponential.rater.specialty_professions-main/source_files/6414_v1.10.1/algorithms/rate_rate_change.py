...
from algorithms.rate_constants import max_layers, expiring_layer_names
...
def rate_rate_change(hxd):
    ...
    for idx, layer in enumerate(layers): 
        ...
        #populate the expiring layer name
        layer.rate_change.expiring_layer_info = "Expiring: " + expiring_layer_names[layer.rate_change.expiring_layer - 1] if layer.rate_change.expiring_layer else "Expiring Layer: Not Selected"
    ...
    if hxd.cds.standard_fields.is_case_priced:
        if layer.rate_change.risk_adjusted_rate_change_case_priced.uw_selected is None and layer.status in ["Bound", "Post Bind Complete"]:
            hx.errors.validation(f"Rate change must be completed for bound layer {idx+1}")
    ...
    # Flag to show additional notifications on the RC page if required. 
    hxd.page_info.rate_change.notification_flag = False
    hxd.page_info.rate_change.notification_text = ""
