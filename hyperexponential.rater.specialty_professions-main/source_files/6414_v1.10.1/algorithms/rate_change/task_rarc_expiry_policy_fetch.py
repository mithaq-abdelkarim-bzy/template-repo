...
def align_data_schema_legacy_model_versions(hxd, expiring_data):
    ...
    # 202608 - DP added mapping from legacy contractor disciplines to current disciplines
    disciplines = ["fp_wet", "hvac_non_industrial", "telecom_light"]
    legacy_disciplines = ["fp", "hvac", "telecom"]
    
    empty_disciplines = ["drywall", "fp_dry", "hvac_industrial", "telecom_heavy"]
    dict_template = {
        "base_premium": 0.0,
        "base_premium_size_discount": 0.0,
        "base_rate_override": {
            "calculated": 1.0,
            "override": None,
            "is_dirty": False,
            "is_overridden": False,
            "overridden_calculated": None,
            "selected": 1.0
        },
        "default_base_rate": 1.0, 
        "guidelines": None, 
        "percentage": 0.0, 
        "rateable_exposure": 0.0, 
        "rateable_exposure_size_discount": 0.0
    }
              
    contractor = expiring_data["cds"]["exposure"]["granular"]["contractor"]

    for d, ld in zip(disciplines, legacy_disciplines):
        if d not in contractor and ld in contractor:
            contractor[d] = contractor[ld].copy()
            del contractor[ld]

    for d in empty_disciplines:
        if d not in contractor:
            contractor[d] = dict_template
