...
function vw_loop_rate_change_layer(num_layers) {
  ...
  for (let n = 0; n < num_layers; n++) {
    let section_title = "Renewal Layer - " + (n == 0 ? "Primary" : "" + n + getOrdinalSuffix(n) + " Excess")
    objects.push(
      <HX.Section title={section_title} defaultCollapsed shownBy={"cds/rate_change/show_layer_" + (n + 1)}>
        <HX.With context={{ type: "list", path: "cds/layers", index: n }}>
          <HX.Pane>
            <HX.Collection fields={["rate_change/expiring_layer_label", null, null]} horizontal />
            <HX.Table
...
function vw_rate_change(scale) {
  return (
    <HX.Page title="Rate Change" shownBy="model_state/show_rate_change" viewScale={scale}>
