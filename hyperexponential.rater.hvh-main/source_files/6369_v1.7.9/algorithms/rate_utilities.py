...
def year_frac(start_date, end_date) -> float:
    ...
    if year_start == year_end:
        year_length = 366 if is_leap_year(year_start) else 365
        return (end_date - start_date).days / year_length

    end_of_start_year = date(year_start + 1, 1, 1)
    start_of_end_year = date(year_end, 1, 1)

    year_length_start = 366 if is_leap_year(year_start) else 365
    year_length_end = 366 if is_leap_year(year_end) else 365

    frac_start = (end_of_start_year - start_date).days / year_length_start
    frac_end = (end_date - start_of_end_year).days / year_length_end
    full_years = year_end - year_start - 1

    return frac_start + full_years + frac_end
...
def signed_percentage_difference(factor1, factor2):
    """
    Calculate the signed percentage difference between two factors

    Returns:
        float or None: Signed percentage difference, or None if either factor is None.
    """
    if factor1 is None or factor2 is None:
        return None
    
    if factor1 == 0:
        return None if factor2 > 0 else None if factor2 < 0 else 0.0

    return ((factor2 - factor1) / factor1 )

def get_lookup_value(table, column, lookup_code, reference_column='Lookup code', error_behavior='raise', default_value=None):
    """
    Retrieves a value from a DataFrame based on a lookup condition.

    Args:
        table : The parameter table to look up from.
        column (str): The column from which to retrieve the value.
        lookup_code (any): The value to match in the reference column.
        reference_column (str, optional): The column to use for lookup. Defaults to 'Lookup code'.
        error_behavior (str, optional): How to handle errors.
            - 'raise' (default) -> Raises validation error if lookup fails.
            - 'default_value' -> Returns `default_value` if lookup fails.
        default_value (any, optional): The value to return if lookup fails when `error_behavior='default_value'`.

    Returns:
        any: The found value or default value based on error behavior.
    """
    filtered_table = table[table[reference_column] == lookup_code]

    if filtered_table.empty:
        if error_behavior == 'default_value':
            return default_value
        else:
            hx.errors.fatal(
                f"Lookup failed: No matching value found for '{lookup_code}' in column '{reference_column}'"
            )

    return filtered_table[column].iloc[0]

def get_minimum_deductible(hxd, peril, state, distance_to_coast,default_value=2500):
    """Retrieve minimum deductible from the corresponding table."""
    min_deductible_table = getattr(hx.params, f'table_minimum_{peril}_deductible', None)
    if min_deductible_table is not None:
        hazard_zone = getattr(hxd.cds.rating_factors, peril).hazard_zone.value
        filtered_table = min_deductible_table[
            (min_deductible_table["State"] == state) &
            (min_deductible_table["Hazard Zone"] == hazard_zone) &
            (min_deductible_table["Distance to Coast"] >= distance_to_coast)
        ]
        return filtered_table[f"Minimum {const.all_perils_dict[peril]} Deductible"].iloc[0] if not filtered_table.empty else default_value
    return default_value  # Default minimum deductible

def get_deductible_impact(peril, deductible_value):
    """Retrieve deductible impact from the deductible load table."""
    deductible_load_table = hx.params.table_deductible_load
    filtered_table = deductible_load_table[deductible_load_table["Lookup code"] <= deductible_value]
    return filtered_table.iloc[-1][const.all_perils_dict[peril]] if not filtered_table.empty else 1